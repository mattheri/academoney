...
import { CalendarContextProvider } from "@/calendar/components/CalendarContextProvider";
import { CalendarTotal } from "@/calendar/components/CalendarTotal";
...
const Calendar: React.FC<CalendarProps> = ({ events, month, year }) => {
  ...
  return (
    <CalendarContextProvider month={month} year={year} events={events}>
          ...
          <thead>
            <tr>
              {[
                ...
                <th key={day} className="border border-white p-2 rounded-md">
                  {day}
                </th>
              ))}
            </tr>
                        ...
                        >
                          ...
                          <div id={day && format(day, "yyyy-MM-dd")}>
                            {allEvents.map((event, eventIndex) => (
                              <p key={eventIndex}>
                                {event.date.toString() === day.toString()
                                  ? event.name
                                  : ""}
                              </p>
                            ))}
          ...
          <ul>
            {allEvents?.length > 0 ? (
              allEvents.map((event, index) => (
                <li key={index}>
                  <strong>{format(event.date, "dd MMMM yyyy")}</strong>:{" "}
                  {event.name}
                </li>
              ))
            ) : (
            ...
            <div className="bg-[#092d74] p-4 rounded-md border-2 border-white">
              ...
              <form action="" className="flex flex-col">
                ...
                <input
                  ...
                  type="text"
                  onChange={(e) => handleEventInput(e)}
                />
              </form>
              <button
                className="bg-[#bf0c1d] rounded-md p-2"
                onClick={() => setIsModalOpen(false)}
              >
                Annuler
              </button>
              <button
                className="bg-[#ef3832] rounded-md p-2 ml-2 mt-2"
